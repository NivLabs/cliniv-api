...
public class PatientService implements GenericService {
	...
	private void handleHealthPlah(PatientInfoDTO entity, Patient patient) {
		if (entity.getHealthPlan() != null
			...
			HealthPlan planFromDb = healthPlanDao.findByPlanCode(entity.getHealthPlan().getId()).orElseThrow(
					() -> new HttpException(HttpStatus.UNPROCESSABLE_ENTITY, "Plano de saúde não cadastrado"));
